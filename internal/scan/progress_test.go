...
func TestEngineProgressLines(t *testing.T) {
	...
	if !strings.Contains(stderr, "elapsed") || !strings.Contains(stderr, "keys seen") || !strings.Contains(stderr, "downloaded") {
		t.Fatalf("progress line missing fields:\n%s", stderr)
	}
...
func TestProgressLineListOnly(t *testing.T) {
	...
	if !strings.Contains(line, "reported") || strings.Contains(line, "downloaded") {
